...
<div class="container">
    ...
    <form action="{{ route('presenceupdate', $presenceGroup->first()->id) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach($presenceGroup as $presence)
                                <tr>
                                    <td>{{ $presence->etudiant->firstname }} {{ $presence->etudiant->lastname }}</td>
                                    <td>
                                        <select class="form-control" name="etudiants[{{ $presence->etudiant_id }}]" required>
                                            <option value="">Sélectionner</option>
                                            <option value="présent" {{ strtolower($presence->type_presence) === 'présent' ? 'selected' : '' }}>Présent</option>
                                            <option value="absent" {{ strtolower($presence->type_presence) === 'absent' ? 'selected' : '' }}>Absent</option>
                                        </select>
                                    </td>
                                </tr>
                            @endforeach
