...
<div class="main-content">
    ...
    <div class="header">
        ...
        <div class="header-actions">
            ...
            <a href="{{ route('admindashboard.notel3create') }}" class="btn btn-success">
    ...
    <div class="container">
        <h2><i class="fas fa-table"></i>  Notes  L3</h2>
        ...
        <div class="table-wrapper">
            <table id="notesTable">
                ...
                <tbody id="tableBody">
                    @forelse ($notesl3 as $notel3)
                        <tr>
                            <td class="student-name nom-etudiant">
                                {{ $notel3->etudiant->firstname }} {{ $notel3->etudiant->lastname }}
                            </td>
                            @for ($ue = 1; $ue <= 5; $ue++)
                                @for ($mat = 1; $mat <= 3; $mat++)
                                    <td class="note-cell {{ getNoteClass($notel3->{"ue{$ue}_m{$mat}_dev1"}) }}">
                                        {{ $notel3->{"ue{$ue}_m{$mat}_dev1"} ?? '-' }}
                                    </td>
                                    <td class="note-cell {{ getNoteClass($notel3->{"ue{$ue}_m{$mat}_dev2"}) }}">
                                        {{ $notel3->{"ue{$ue}_m{$mat}_dev2"} ?? '-' }}
                                    </td>
                                    <td class="note-cell {{ getNoteClass($notel3->{"ue{$ue}_m{$mat}_exam"}) }}">
                                        {{ $notel3->{"ue{$ue}_m{$mat}_exam"} ?? '-' }}
                                    </td>
                                @endfor
                            @endfor
                            <td class="actions-cell">
                                <a href="{{ route('admindashboard.notel3edit', $notel3) }}" class="btn btn-sm btn-primary">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('notel3destroy', $notel3) }}" method="POST" 
