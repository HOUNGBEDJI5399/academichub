...
@if ($presencesl3->count() > 0)
    ...
    @foreach($groupedPresences as $key => $group)
        ...
        <div class="presence-sheet">
            ...
            <div class="actions">
                <a class="btn btn-primary" href="{{ route('professeurdashboard.presencel3edit', $first) }}">Éditer</a>
      <form action="{{ route('presencel3destroy', $first) }}" method="POST" style="display: inline;">
                    @csrf
                    @method('DELETE')
