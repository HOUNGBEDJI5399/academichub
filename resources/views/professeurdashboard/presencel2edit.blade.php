...
<div class="container">
    ...
    <form action="{{ route('presencel2update', $presenceGroup->first()->id) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach($presenceGroup as $presencel2)
                                <tr>
                                    <td>{{ $presencel2->etudiant->firstname }} {{ $presencel2->etudiant->lastname }}</td>
                                    <td>
                                        <select class="form-control" name="etudiants[{{ $presencel2->etudiant_id }}]" required>
                                            <option value="">Sélectionner</option>
                                            <option value="présent" {{ strtolower($presencel2->type_presence) === 'présent' ? 'selected' : '' }}>Présent</option>
                                            <option value="absent" {{ strtolower($presencel2->type_presence) === 'absent' ? 'selected' : '' }}>Absent</option>
                                        </select>
                                    </td>
                                </tr>
                            @endforeach
